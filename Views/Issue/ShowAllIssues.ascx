...
<div id="issuesList">
    ...
    <Span style="float:right;" title="Search issue by id">[ <%: Html.ActionLink("Search View", "AddIssueDependency", "Issue", null, new { id = "searchViewLink" })%> ]</Span>
    <% using (Html.BeginForm("AddIssueDependencyList", "Issue"))
        {
    %>
        <table class="bodyTable" id="issueListTable">
            <% if (Model != null)
                ...
                <thead title="Click to Sort the List"> 
                    <tr>
                        <th>
                            <h3>Select the depends on Issue(s).</h3>
                        </th>
                    </tr>
                    </thead>
